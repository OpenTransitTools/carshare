...
class Vehicle(Base):
    ...
    def update_position(self, session, lat, lon, address=None, city=None, zipcode=None, neighborhood=None, time_span=144):
        ...
        try: 
            if p is None:
                ...
                p.set_position(lat, lon, address, zipcode, city, neighborhood)
                session.add(p)
                session.flush()
                session.commit()
            else:
